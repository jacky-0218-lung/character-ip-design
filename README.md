...
> **名稱說明**：plugin／marketplace 名稱是 `character-ip-design`（指令穩定性），skill 本身的
> 名稱則是 `designing-character-ips`，依 Anthropic 對 skill `name` 使用動名詞形的建議。兩者
> 刻意不同，安裝指令不受影響。
...
> 請從 `https://github.com/jacky-0218-lung/character-ip-design/tree/main/skills/designing-character-ips` 安裝 `designing-character-ips` 這個 skill。優先用公開 repository 的直接下載；只有在直接下載因權限問題失敗時才退回 git clone。只安裝 `skills/designing-character-ips` 這個子目錄到我 Agent 信任的 skills 目錄。若目的地已存在，請停止並回報，不要覆蓋。安裝完成後回報安裝路徑，並告訴我何時可以開始使用。
...
2. **初始化表單**：Agent 用一張選擇題表單一次問齊 9 題（品牌、受眾、個性、應用階梯、風格、萌度、既有資產、命名、完成度）。也可以打開 [`skills/designing-character-ips/assets/intake-form.html`](skills/designing-character-ips/assets/intake-form.html) 自己填，按「產生需求摘要」把摘要貼回對話。
...
python3 tools/run_evals.py list          # 行為評測情境
python3 tools/run_evals.py pack          # 產生各模型的評測執行單
```
...
> **On the two names**: the plugin/marketplace is `character-ip-design` (stable install
> commands); the skill itself is `designing-character-ips`, following Anthropic's gerund-form
> guidance for skill `name`. The difference is deliberate and does not affect installation.
...
> Install the `designing-character-ips` skill from `https://github.com/jacky-0218-lung/character-ip-design/tree/main/skills/designing-character-ips`. Prefer direct download for this public repository and fall back to git clone only on permission errors. Install only the `skills/designing-character-ips` subtree into my agent's trusted skills directory. If the destination already exists, stop and report it instead of overwriting. Report the installed path when done.
...
You ask to make a character → the skill presents a **one-round intake form** (9 questions), or you fill [`skills/designing-character-ips/assets/intake-form.html`](skills/designing-character-ips/assets/intake-form.html) and paste the generated brief back. Then the pipeline runs and delivers one self-contained **IP bible (HTML with every asset inlined)** plus SVG masters and per-surface exports.
